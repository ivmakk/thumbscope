...
- **Menubar** (`src/renderer/src/components/MenuBar.tsx` + `ui/menubar.tsx`): `bg-card`, `rounded-sm` items, tight padding; top-level triggers highlight on `hover`/`focus`/open via `--accent`. Custom in-renderer bar replaces the native Win32 menu (which can't match this look). **Keyboard access:** on Windows/Linux a clean Alt *tap* latches keyboard-access mode - the access letter in each trigger (File/Edit/View/Window/Help) underlines and stays underlined, the bar takes focus, `Alt+letter` (or a bare letter while latched) opens a menu, arrows navigate, and Esc/click/item-select exits and restores prior focus. Underlines show *only* in this mode (not the OS "always show access keys" setting). macOS has no underline idiom: Ctrl+F2 focuses the bar (Tab+arrows is the universal baseline). The latch is a pure reducer (`lib/altMode.ts`); access letters live in `lib/menus.ts`. Dev-only items (Reload, Toggle Developer Tools) are hidden outside dev builds; a Help menu carries external links (opened via an https-guarded `openExternal`) and a disabled version label.
...
- **Browse table** (`src/renderer/src/components/BrowseTable.tsx`, kernel `lib/tableColumns.ts`): columns are `#` · Name · Size · Date · Dims, with a trailing `1fr` spacer. Name flexes (`minmax(12rem, 36rem)`) so it absorbs slack on small/mid windows but caps on wide ones - the spacer collects the leftover on the far right, keeping the metadata columns grouped by the name instead of drifting to the edge. A single scroll container holds a `sticky top-0` header and the virtualized body so header and rows share one horizontal scroll (aligned when a wide column forces overflow); both carry `min-width: minTemplateWidth()` so the grid fills when wide and overflows together when narrow. Columns resize by dragging the right-edge handle (push model - only the grabbed column changes) and **auto-fit on double-click**, measured from real rendered text width (canvas `measureText`, header font vs cell font), not a char count. Widths are ephemeral: they reset to the adaptive default on every new file open. Empty Date/Dims render blank (no dash); the `#` column falls back to a stable 1-based positional stamp (position in the parser's entry order) for variants with no item ID.
- **Filename truncation** (`src/renderer/src/components/FileName.tsx`, used by the grid tile and table name cell): font-accurate middle ellipsis done in CSS, not by counting characters. No real extension -> plain end ellipsis (`truncate`). Has extension -> flex two-span: the head `truncate`s, a protected tail (extension + up to 4 trailing stem digits, e.g. `IMG_…0123.jpg`) stays. Full name on hover via the native `title`.
...
- **Preview** (`src/renderer/src/components/Preview.tsx`): top-to-bottom order is image well → compact control bar → info status bar. Image well uses a theme-independent tint (`bg-foreground/[0.04]`); control buttons are compact (`h-6 px-2`); Fit and 1:1 show the `default` (filled) variant when active, `outline` otherwise. The bar is responsive: a `ResizeObserver` collapses controls into a trailing "..." overflow menu, in priority order, when the inline row actually overflows (`scrollWidth > clientWidth`, not a hardcoded width), so nothing clips at any panel width. Rotate left/right collapse first, then zoom out/in; fit and 1:1 always stay inline. Kebab uses a lucide `MoreHorizontal` icon with an `aria-label`; menu items pair an icon with visible text.
- **Dropdown / overflow menu** (`ui/dropdown-menu.tsx`): vendored shadcn primitive (Radix `react-dropdown-menu`), styled to match `ui/menubar.tsx` - elevated `bg-card` surface, hairline border + `shadow-md` (Level 1 elevation), `rounded-sm` items, `--accent` focus/hover, monochrome. Items pair a lucide icon with visible text.
