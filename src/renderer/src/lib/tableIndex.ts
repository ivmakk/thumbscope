...
// input (file / parse) order. Feed this the RAW entries in file order, before any sort/filter,
// so the stamp is an absolute file position that then travels with the row when sorted.
// Pure, no DOM/React -> unit-tested under `node --test`. Renderer-side only (see task doc for the
// CLI-parity note).
