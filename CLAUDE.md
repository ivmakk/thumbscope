...
**SQLite caches (`sqlite-photothumb`) are not OLE2** and live outside `parser.ts`: `photothumb.ts` reads them with Node's built-in `node:sqlite` (zero deps). `DatabaseSync` opens a *path*, not a buffer, so `isSqliteFile(path)` routes at the call site (main `openPath` / CLI `load`) by reading just the 16-byte header - if the magic matches, `parsePhotothumb(path)` runs; otherwise the full buffer is read and `parseThumbsDb(buf)` runs (so the OLE2 path isn't loaded into memory for a magic check). `node:sqlite` is Node-only (like `cfb`/`sharp`), available in every runtime we use (Electron's Node, the CLI via `ELECTRON_RUN_AS_NODE`, `node --test`) but never imported by the renderer, so it stays out of the sandboxed page just as `parser.ts` does.
