...
- **Bug PRs**: when relevant, state the **file variant** (`catalog-jpeg` / `catalog-jpeg-guid` / `hashed-jpeg` / `abbrev-jpeg` / `catalog-dib` / `irfanview-flat` / `irfanview-nested` / `recovered`) the fix targets - see the format taxonomy in `CLAUDE.md`.
